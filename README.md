...
## 진행 중인 작업 — 디자인 시스템 & Storybook

학습을 겸해 디자인 시스템과 Storybook을 도입하는 작업을 진행하고 있습니다.

- **Storybook 도입**: `shared/ui` 컴포넌트부터 스토리를 작성하며, 격리된 환경에서 컴포넌트를 개발·검증하는 워크플로를 적용 중입니다.
- **디자인 토큰 점검**: `app/globals.css`의 색상·radius 토큰을 실제 사용 여부 기준으로 다시 확인하고, 죽은 토큰이나 오타를 바로잡는 중입니다.
- **재사용 신호 기반 컴포넌트 리팩토링**: prop과 variant를 실제 소비자 사용 여부로 검증해, 쓰이지 않는 건 제거하고 필요한 것만 남기는 방식으로 `shared/ui` 컴포넌트를 정리하고 있습니다.
- **컴포넌트 API 설계 학습**: Radix UI, Chakra UI, MUI 등 주요 라이브러리의 구현을 참고해 접근성(fallback 처리, 이름 기반 alt 텍스트)과 스타일 확장성(닫힌 variant vs 배치용 className) 설계 원칙을 적용해보고 있습니다.
