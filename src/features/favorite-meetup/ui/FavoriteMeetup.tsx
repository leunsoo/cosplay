...
export function FavoriteMeetup({
  title,
  month,
  day,
  status,
  ...
  return (
    ...
    >
      <div className="flex flex-col items-center bg-gray-100 rounded-lg px-2 py-1 border border-gray-200 min-w-11">
        <span
          className={`text-[10px] font-bold ${status === '진행중' ? 'text-primary' : 'text-gray-500'}`}
        >
          {month}
        </span>
        <span
          className={`text-base font-black leading-tight ${status === '진행중' ? 'text-slate-800' : 'text-slate-600'}`}
        >
          {day}
        </span>
      </div>
